...
<div class="createForm">
    ...
    <input type="text" name="nickname" id="nickname" value="{{ $edit->nickname ?? old('nickname') }}" placeholder="Digite aqui o nickname do funcionario">
    
    <label for="birth_date">Data aniversario:</label>
    <input type="date" name="birth_date" id="birth_date" value="{{ $edit->birth_date ?? old('birth_date') }}" placeholder="Digite a data do seu aniversario">
    ...
    <select name="position_id" id="position_id">
        ...
        @foreach ($positions as $position)
            <option value="{{ $position->id }}" 
                @if (isset($edit) && $position->id === $edit->position_id)
                    selected
                @endif
                >
    ...
    <input type="file" name="image" id="image" value="{{ $edit->image ?? old('image') }}" placeholder="Insiria a foto">
    ...
    <textarea name="description" id="description" cols="30" rows="4" placeholder="Digite aqui a descrição do funcionario">{{ $edit->description ?? old('description') }}</textarea>
